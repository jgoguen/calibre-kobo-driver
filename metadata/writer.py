...
class KEPUBMetadataWriter(EPUBMetadataWriter):
    ...
    def set_metadata(self, stream, mi, type):
        ...
        if len(covers) > 0:
            ...
            default_log(
                "KEPUBMetadataWriter::set_metadata - cover_id={0}".format(
                    cover_id
                )
            )
            ...
            if not found_cover:
                ...
                for item in reader.opf.itermanifest():
                    if item.get('href', None) == cover_id:
                        ...
                        if mt and mt.startswith('image/'):
                            default_log(
                                "KEPUBMetadataWriter::set_metadata -found "
                                "cover"
                            )
                            item.set("properties", "cover-image")
                            found_cover = True
            ...
            if found_cover:
                ...
                if isinstance(reader.archive, LocalZipFile):
                    reader.archive.safe_replace(
                        reader.container[OPF.MIMETYPE], newopf)
                else:
                    safe_replace(
                        stream, reader.container[OPF.MIMETYPE], newopf)
